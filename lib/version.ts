const repositoryUrl =
    process.env.NEXT_PUBLIC_GITHUB_REPO_URL ?? 'https://github.com/rhymednick/k7rhy.app';
...
const commitUrl =
    normalizedRepoUrl &&
    process.env.NEXT_PUBLIC_GIT_COMMIT_IS_PUBLIC === 'true' &&
    /^[0-9a-f]{7,40}$/i.test(commitHash)
        ? `${normalizedRepoUrl}/commit/${commitHash}`
        : undefined;

const commitLinkLabel = shortCommitHash;

const parsedBuildDate =

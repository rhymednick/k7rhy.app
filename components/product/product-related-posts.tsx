...
export function ProductRelatedPosts({ product, relatedPosts }: ProductRelatedPostsProps) {
                ...
                <SheetContent side="right" className="w-full sm:max-w-2xl overflow-y-auto">
                    <SheetHeader>
                        ...
                        <SheetDescription>
                            ...
                            {relatedPosts.length !== 1 ? 's' : ''} related to this product
                        </SheetDescription>
                    </SheetHeader>
                        ...
                        <div className="mb-4 pb-4 border-b">
                            ...
                            <div className="space-y-1 max-h-32 overflow-y-auto">
                                {relatedPosts.map((post, index) => (
                                    <button key={post._meta.path} onClick={() => setCurrentPostIndex(index)} className={`w-full text-left px-2 py-1 rounded text-sm ${index === currentPostIndex ? 'bg-primary text-primary-foreground' : 'hover:bg-muted'}`}>
                                        {post.title}
                                    </button>
                                ))}
                            </div>
                    ...
                    <div className="mt-4">
                        <div className="mb-4">
                            ...
                            <div className="text-sm text-muted-foreground mb-4">
                                <time dateTime={new Date(currentPost.date).toISOString()}>{formatDate(currentPost.date)}</time>
                            </div>
                            {currentPost.tags && currentPost.tags.length > 0 && (
                                <div className="flex flex-wrap gap-2 mb-4">
                                    {currentPost.tags.map((tag) => (
                                        <span key={tag} className="inline-block bg-green-100 text-green-700 text-xs font-medium px-2 py-0.5 rounded">
                                            #{tag}
                                        </span>
                                    ))}
                                </div>
                            )}
                            {currentPost.summary && <div className="prose prose-slate dark:prose-dark mb-4">{currentPost.summary}</div>}
                            <Link href={`/blog/${currentPost._meta.path}`} className="text-blue-600 dark:text-blue-400 hover:underline">
                                Read full post →
                            </Link>
                        </div>
                    </div>

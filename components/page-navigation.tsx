...
export const PageNavigation = () => {
    ...
    useEffect(() => {
        ...
        const filteredHeadings = h2AndH3Elements.filter(
            (heading) =>
                !heading.hasAttribute('exclude-from-page-nav') &&
                            ...
                            <ul className="space-y-2 p-4 ">
                                {headings.map((heading, index) => {
                                    ...
                                    return (
                                        <li
                                            key={index}
                                            className={`pl-${isH2 ? 0 : 4}`}
                                        >
                                            <a
                                                href={`#${heading.id}`}
                                                className={`text-sm font-medium ${
                                                    isH2
                                                        ? 'dark:text-gray-500 text-gray-700 dark:hover:text-blue-400 hover:text-blue-600'

    ...
    return (
        <div role="tablist" className="mb-6 flex flex-wrap gap-1 border-b">
            {voicings.map((voicing) => {
                ...
                return (
                    <Link key={voicing.slug} href={`${basePath}?voicing=${voicing.slug}`} scroll={false} role="tab" aria-current={isActive ? 'page' : undefined} aria-selected={isActive} className={cn('-mb-px border-b-2 px-3 py-2 text-sm font-medium transition-colors', isActive ? 'border-sky-500 text-foreground' : 'border-transparent text-muted-foreground hover:border-border hover:text-foreground')}>
                        {voicing.name}
                    </Link>
                );
            })}

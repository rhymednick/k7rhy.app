...
const renderSubsteps = (substeps?: DocProcedureSubstep[]): React.ReactNode => {
    ...
    return (
        ...
        >
            {substeps.map(
                (substep: DocProcedureSubstep, substepIndex: number) => (
                    <li
                        key={substepIndex}
...
export const DocProcedure: React.FC<DocProcedureProps> = (
    props: DocProcedureProps
) => {
    const renderStep = (
        step: DocProcedureStep,
        stepIndex: number
    ): React.ReactNode => {
        const description: React.ReactNode = step.description
            ? step.description
            : null;
        const image: React.ReactNode = step.image ? (
            <DocImage
                src={step.image}
                title={step.text}
                alt={step.text}
    ...
    return (
        <div>
            <DocSection title={props.title}>
                ...
                >
                    {props.docProcedureSteps?.map((step, index) =>

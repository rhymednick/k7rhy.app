...
export function ControlPositions({ switchLabel, children }: ControlPositionsProps) {
    return (
        <div className="my-4">
            {switchLabel && <p className="mb-2 text-xs font-semibold uppercase tracking-widest text-muted-foreground">{switchLabel}</p>}
    ...
    return (
        <div className="flex gap-3 px-4 py-3">
            <div className="shrink-0 pt-0.5">
                <span className={cn('inline-flex min-w-[26px] items-center justify-center rounded-full px-1.5 py-0.5', 'bg-indigo-100 text-xs font-semibold tabular-nums text-indigo-700', 'dark:bg-indigo-900/60 dark:text-indigo-300')}>{label}</span>
            </div>

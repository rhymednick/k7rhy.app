...
export function DocsSidebarNavItems({ items, pathname }: DocsSidebarNavItemsProps) {
    ...
    function renderNavItem(item: NavItemWithChildren, index: number) {
        ...
        if (item.href) {
            ...
            return (
                <span key={index} className={cn('flex w-full items-center rounded-md p-2 text-muted-foreground hover:underline')}>
                    {item.title}
    ...
    return items?.length ? <div className="grid grid-flow-row auto-rows-max text-sm">{items.map(renderNavItem)}</div> : null;
}

    ...
    return (
        <nav className="flex items-center">
            <div className="relative overflow-hidden">
                <div id="socialsDrawer" className={`flex items-center transition-transform duration-500 ${isOpen ? 'translate-x-0' : 'translate-x-[150%]'}`}>
                    {socials.map((social, index) => (
                        <SocialIcon key={social.key} target="_blank" url={social.url} style={{ height: 25, width: 25 }} className={index === 0 ? '' : 'ml-2'} />
                    ))}
                </div>
            </div>
            ...
            <div className="mr-4 text-foreground/60 text-sm">Social Networks</div>

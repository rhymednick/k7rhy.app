...
import { ControlLayout, InstrumentSpec, Pickup, PickupConfiguration, PickupDetail, Pot, PotPosition, Selector, SelectorPosition, Toggle, ToggleState } from '@/components/instrument/instrument-spec';
...
const components: MDXComponents = {
    ...
    a: ({ className, children, ...props }: React.HTMLAttributes<HTMLAnchorElement>) => (
        <a className={cn('font-medium no-underline hover:underline underline-offset-2', className)} {...props}>
            {children}
        </a>
    ),
    p: ({ className, ...props }: React.HTMLAttributes<HTMLParagraphElement>) => <p className={cn('prose [&:not(:first-child)]:mt-2', className)} {...props} />,
    ul: ({ className, ...props }: React.HTMLAttributes<HTMLUListElement>) => <ul className={cn('my-2 ml-6 list-disc', className)} {...props} />,
    ol: ({ className, ...props }: React.HTMLAttributes<HTMLOListElement>) => <ol className={cn('my-2 ml-6 list-decimal', className)} {...props} />,
    li: ({ className, ...props }: React.HTMLAttributes<HTMLElement>) => <li className={cn('', className)} {...props} />,
    ...
    img: ({ className, alt, src, width, height, ...rest }: React.ImgHTMLAttributes<HTMLImageElement>) => {
        const numericWidth = typeof width === 'number' ? width : width ? Number(width) : 800;
        const numericHeight = typeof height === 'number' ? height : height ? Number(height) : 600;
        ...
        return <Image className={cn('rounded-md mt-2 mb-2', className)} alt={alt ?? ''} src={imageSrc} width={numericWidth} height={numericHeight} {...rest} />;
    },
    ...
    tr: ({ className, ...props }: React.HTMLAttributes<HTMLTableRowElement>) => <tr className={cn('m-0 border-t p-0', className)} {...props} />,
    ...
    TabsList: ({ className, ...props }: React.ComponentProps<typeof TabsList>) => <TabsList className={cn('w-full justify-start rounded-none border-b bg-transparent p-0', className)} {...props} />,
    TabsTrigger: ({ className, ...props }: React.ComponentProps<typeof TabsTrigger>) => <TabsTrigger className={cn('relative h-9 rounded-none border-b-2 border-b-transparent bg-transparent px-4 pb-3 pt-2 font-semibold text-muted-foreground shadow-none transition-none data-[state=active]:border-b-primary data-[state=active]:text-foreground data-[state=active]:shadow-none', className)} {...props} />,
    TabsContent: ({ className, ...props }: React.ComponentProps<typeof TabsContent>) => <TabsContent className={cn('relative [&_h3.font-heading]:text-base [&_h3.font-heading]:font-semibold', className)} {...props} />,

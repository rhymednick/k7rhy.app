...
const BlogPostItem: React.FC<BlogPostItemProps> = ({ post }) => {
    return (
        <article className="relative group">
            ...
            {post.publish === false && <div className="absolute inset-0 flex items-center justify-center opacity-10 text-red-500 font-bold text-6xl z-0">Unpublished</div>}
            <div className="relative z-10">
                <div className="flex flex-col">
                    <h3 className={`text-base md:text-xl font-semibold tracking-tight ${post.publish === false ? 'opacity-60' : ''} text-slate-900 dark:text-slate-200 pt-8 lg:pt-0`}>{post.title}</h3>
                    {post.tags && (
                        <div className="mt-1 space-x-2 text-xs text-slate-400 dark:text-slate-500 flex flex-wrap">
                            {post.isAISummary && <span className="inline-block bg-blue-100 text-blue-700 text-xs font-medium mr-1 px-2 py-0.5 rounded">#AI-generated Summary</span>}
                            {post.tags.map((tag) => (
                                <span key={tag} className="inline-block bg-green-100 text-green-700 text-xs font-medium mr-1 px-2 py-0.5 rounded">
                                    #{tag}
                                </span>
                            ))}
                        </div>
